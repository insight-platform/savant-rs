//! Integration tests for [`SurfaceView::from_shared`] and batched buffer access.
//!
//! These tests verify slot-level access to batched buffers via the new API:
//! - `SurfaceView::from_shared(&shared, slot_index)` for zero-copy GPU views
...
fn build_uniform_batch(ids: &[i64]) -> SharedMutableGstBuffer {
    ...
    batch.finalize(ids.len() as u32, id_kinds).unwrap();
...
fn test_uniform_view_last_slot() {
    common::init();
    let shared = build_uniform_batch(&[10, 20, 30]);
    let view = SurfaceView::from_shared(&shared, 2).unwrap();
...
fn test_uniform_slot_out_of_bounds() {
    ...
    assert!(SurfaceView::from_shared(&shared, 2).is_err());
    assert!(SurfaceView::from_shared(&shared, 100).is_err());
}
...
fn test_heterogeneous_view_preserves_dimensions() {
    ...
    for (i, &(w, h)) in resolutions.iter().enumerate() {
        let view = SurfaceView::from_shared(&shared, i as u32).unwrap();
...
fn test_heterogeneous_slot_out_of_bounds() {
    ...
    assert!(SurfaceView::from_shared(&shared, 1).is_err());
}
...
fn test_uniform_shared_valid_after_cow() {
    ...
    let shared = {
        ...
        batch.finalize(2, id_kinds).unwrap();
        let shared = batch.shared_buffer();
        {
            let mut guard = shared.lock();
            guard
...
fn test_uniform_slot_view_from_detached_shared() {
    ...
    for i in 0..3u32 {
        ...
        assert!(
            !view.data_ptr().is_null(),
            "slot_view({}) dataPtr must be valid on detached shared",
            i
        );
        assert_eq!(view.width(), 640);
    }
...
fn test_heterogeneous_shared_valid_after_finalize() {
    ...
    let shared = {
        ...
        for (i, &(w, h)) in resolutions.iter().enumerate() {
            let gen = make_src_gen(VideoFormat::RGBA, w, h);
            let s = gen.acquire_buffer(None).unwrap();
            let v = SurfaceView::from_shared(&s, 0).unwrap();
            batch.add(&v, Some(i as i64 + 1)).unwrap();
            id_kinds.push(SavantIdMetaKind::Frame(i as i64 + 1));
...
fn test_uniform_surface_view_no_pool_leak() {
    ...
    for _ in 0..LEAK_ITERATIONS {
        ...
        batch.finalize(2, id_kinds).unwrap();
        let shared = batch.shared_buffer();
        let _v0 = SurfaceView::from_shared(&shared, 0).unwrap();
        let _v1 = SurfaceView::from_shared(&shared, 1).unwrap();
    }
}
...
fn test_heterogeneous_surface_view_no_leak() {
    ...
    for _ in 0..LEAK_ITERATIONS {
        ...
        let _view0 = SurfaceView::from_shared(&shared, 0).unwrap();
        let _view1 = SurfaceView::from_shared(&shared, 1).unwrap();
        drop(v1);
        drop(s1);
        drop(v2);
        drop(s2);
