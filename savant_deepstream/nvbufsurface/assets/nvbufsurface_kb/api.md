...
| `upload_to_surface` | `unsafe (buf: &gst::Buffer, data: &[u8], width: u32, height: u32, channels: u32) → Result<(), NvBufSurfaceError>` | Upload CPU pixel data to the first surface in buf. `channels` must match the surface's color format. Row-by-row copy respecting GPU pitch. Platform-aware. |
