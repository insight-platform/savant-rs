...
use deepstream_nvbufsurface::{
    cuda_init, DsNvSurfaceBufferGenerator, DsNvUniformSurfaceBufferGenerator, NvBufSurfaceMemType,
...
fn make_gen(pool_size: u32) -> DsNvSurfaceBufferGenerator {
    DsNvSurfaceBufferGenerator::builder(VideoFormat::RGBA, 1920, 1080)
        .gpu_id(0)
...
fn bench_batched_multi_slot_views(c: &mut Criterion) {
    ...
    let src_gen = DsNvSurfaceBufferGenerator::builder(VideoFormat::RGBA, 128, 96)
        .gpu_id(0)
        .mem_type(NvBufSurfaceMemType::Default)
    ...
    c.bench_function("surface_view_batched_4slot", |b| {
        b.iter_with_setup(
            ...
            |buf| {
                ...
                for i in 0..batch_size {
                    let view = SurfaceView::from_shared(shared.clone(), i).unwrap();
                    assert!(!view.data_ptr().is_null());
                }
            },
        );
