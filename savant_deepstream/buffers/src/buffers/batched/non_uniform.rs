...
//!
//! [`NonUniformBatch`] assembles individual [`SurfaceView`] slots
...
use crate::{ffi, NvBufSurfaceError, SavantIdMetaKind};
...
///     NonUniformBatch, BufferGenerator,
///     NvBufSurfaceMemType, SavantIdMetaKind, SurfaceView, VideoFormat,
/// };
///
/// gstreamer::init().unwrap();
///
/// let gen_1080p = BufferGenerator::new(
///     VideoFormat::RGBA, 1920, 1080, 30, 1, 0, NvBufSurfaceMemType::Default,
/// ).unwrap();
/// let gen_720p = BufferGenerator::new(
///     VideoFormat::RGBA, 1280, 720, 30, 1, 0, NvBufSurfaceMemType::Default,
/// ).unwrap();
///
/// let buf_1080p = gen_1080p.acquire(Some(1)).unwrap();
/// let buf_720p = gen_720p.acquire(Some(2)).unwrap();
///
/// let view_1080p = SurfaceView::from_buffer(&buf_1080p, 0).unwrap();
/// let view_720p = SurfaceView::from_buffer(&buf_720p, 0).unwrap();
///
/// let mut batch = NonUniformBatch::new(0);
/// batch.add(&view_1080p).unwrap();
/// batch.add(&view_720p).unwrap();
...
impl NonUniformBatch {
    ...
    /// [`SharedBuffer`](crate::SharedBuffer) to keep GPU memory alive via
    /// `GstParentBufferMeta` at finalize time.
    pub fn add(&mut self, src: &crate::SurfaceView) -> Result<(), NvBufSurfaceError> {
        let guard = src.gst_buffer();
        let src_surf = unsafe {
    ...
    /// and return a [`SharedBuffer`](crate::SharedBuffer).
    ///
    /// # Arguments
    ///
