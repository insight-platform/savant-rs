...
/// from upstream pipeline stages from reaching inference.
///
/// ## After
/// When the `BatchInferenceOutput` is **dropped**, all `NvDsObjectMeta`
/// entries are removed from the **output** buffer's frames. This keeps the
/// buffer clean for any downstream consumers and returns objects to the pool.
///
/// *Safety note*: `TensorView` stores raw pointers into
/// `NvDsInferTensorMeta`. Those pointers are invalidated when the objects are
/// cleared. Always consume (or drop) every `TensorView` *before* dropping the
/// owning `BatchInferenceOutput`.
///
/// ## Both
