...
## Test File Location
Integration tests: `savant_deepstream/buffers/tests/*.rs`
...
fn build_heterogeneous_batch(resolutions: &[(u32, u32)], ids: &[i64]) -> SharedBuffer {
    ...
    for (i, &(w, h)) in resolutions.iter().enumerate() {
        ...
        let shared = gen.acquire(Some(ids[i])).unwrap();
        let view = SurfaceView::from_buffer(&shared, 0).unwrap();
...
### Basic Slot View via SurfaceView::from_buffer
```rust
#[test]
...
fn test_uniform_buffer_valid_after_struct_drop() {
    ...
    let shared = {
        let ids: Vec<SavantIdMetaKind> = (0..3).map(SavantIdMetaKind::Frame).collect();
        let mut batch = batched_gen.acquire_batch(TransformConfig::default(), ids).unwrap();
        for i in 0..3u32 {
...
fn test_uniform_buffer_valid_after_cow() {
    ...
    let shared = {
        let ids: Vec<SavantIdMetaKind> = (0..2).map(SavantIdMetaKind::Frame).collect();
        let mut batch = batched_gen.acquire_batch(TransformConfig::default(), ids).unwrap();
        for i in 0..2u32 {
            let src = src_gen.acquire(None).unwrap();
            let src_view = SurfaceView::from_buffer(&src, 0).unwrap();
...
fn test_uniform_no_pool_leak() {
    ...
    for _ in 0..50 {
        ...
        let mut batch = batched_gen
            .acquire_batch(TransformConfig::default(), ids)
            .unwrap();
        for i in 0..2u32 {
            let src = src_gen.acquire(None).unwrap();
            let src_view = SurfaceView::from_buffer(&src, 0).unwrap();
...
fn test_uniform_already_finalized_guards() {
    ...
    assert!(matches!(
        batch.transform_slot(1, &src_view, None),
        Err(NvBufSurfaceError::AlreadyFinalized)
    ));
}
```
...
fn test_uniform_batch_slot_views_distinct() {
    ...
    let src_gen = /* ... */;
    let batch_gen = /* ... */;
    let mut batch = batch_gen.acquire_batch(TransformConfig::default(), ids).unwrap();
    // Fill slots, then finalize()
    let shared = batch.shared_buffer();
    // Create one SurfaceView per slot via from_buffer (takes &shared)
    let views: Vec<_> = (0..N)
        .map(|i| SurfaceView::from_buffer(&shared, i).unwrap())
        .collect();
