...
lazy_static! {
    static ref REGISTERED_HANDLERS: Mutex<HashMap<String, Py<PyAny>>> = Mutex::new(HashMap::new());
}

#[pyfunction]
pub fn register_handler(py: Python, name: &str, handler: &Bound<'_, PyAny>) -> PyResult<()> {
    let mut handlers = REGISTERED_HANDLERS.lock().unwrap();
    let unbound = handler.unbind(py);
    handlers.insert(name.to_string(), unbound);
    Ok(())
}

pub fn get_handler(name: &str) -> Option<&Py<PyAny>> {
    let handlers = REGISTERED_HANDLERS.lock().unwrap();
    handlers.get(name)
}
