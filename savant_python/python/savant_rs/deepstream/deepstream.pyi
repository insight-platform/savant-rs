...
# ── Multi-stream decoder (``deepstream_inputs``) ────────────────────────────


@final
class EvictionVerdict:
    """Idle-eviction verdict from ``on_eviction`` callback."""

    @staticmethod
    def approve() -> EvictionVerdict: ...
    @staticmethod
    def extend(timeout_ms: int) -> EvictionVerdict: ...

@final
class SubmitResult:
    """Queue depth after a successful ``submit`` / ``try_submit``."""
...
class StopReason:
    ...
    def is_idle_eviction(self) -> bool: ...
    def is_codec_changed(self) -> bool: ...
    def is_resolution_changed(self) -> bool: ...
    def is_timestamp_regressed(self) -> bool: ...
    def is_shutdown(self) -> bool: ...
    def error_message(self) -> Optional[str]: ...

@final
class UndecodedReason:
    def is_unsupported_codec(self) -> bool: ...
    def unsupported_codec_name(self) -> Optional[Optional[str]]: ...
    def is_awaiting_keyframe(self) -> bool: ...
    def detection_failed_message(self) -> Optional[str]: ...
    def decode_error_message(self) -> Optional[str]: ...
    def is_stream_evicted(self) -> bool: ...
    def is_session_reset(self) -> bool: ...
    def is_no_payload(self) -> bool: ...
    def is_external_content(self) -> bool: ...

@final
class DecoderOutput:
    """Unified callback payload from ``MultiStreamDecoder``."""
    ...
    def is_pipeline_restarted(self) -> bool: ...
    def as_decoded(self) -> Tuple[VideoFrame, SharedBuffer]: ...
    def as_undecoded(self) -> Tuple[VideoFrame, Optional[Any], UndecodedReason]: ...
    def as_eos(self) -> str: ...
    def as_stream_started(self) -> Tuple[str, Codec]: ...
    def as_stream_stopped(self) -> Tuple[str, StopReason]: ...
    def as_pipeline_restarted(self) -> Tuple[str, str, int]: ...

@final
class MultiStreamDecoderConfig:
    """Per-stream RGBA pool dimensions are derived from each ``VideoFrame``'s width/height
    (and rebuilt after resolution-change session resets), so there is no global
    ``output_width`` / ``output_height``.

    ``interpolation`` selects the NvBufSurfTransform filter for NVMM→RGBA (1:1 into the pool);
    same ``Interpolation`` enum as ``TransformConfig`` (default ``Interpolation.BILINEAR``).
    """
    ...
    def __init__(
        ...
        interpolation: Interpolation = ...,
        idle_timeout_ms: int = ...,
        max_detection_buffer: int = ...,
        per_stream_queue_size: int = ...,
        session_eos_on_codec_change: Optional[bool] = None,
        session_eos_on_resolution_change: Optional[bool] = None,
        session_eos_on_timestamp_regress: Optional[bool] = None,
    ) -> None: ...
    @property
    def gpu_id(self) -> int: ...
    @property
    ...
    @property
    def max_detection_buffer(self) -> int: ...
    @property
    def per_stream_queue_size(self) -> int: ...
    @property
    def session_eos_on_codec_change(self) -> bool: ...
    @property
    def session_eos_on_resolution_change(self) -> bool: ...
    @property
    def session_eos_on_timestamp_regress(self) -> bool: ...
...
class MultiStreamDecoder:
    def __init__(
        ...
        on_output: Callable[[DecoderOutput], None],
        on_eviction: Optional[Callable[[str], EvictionVerdict]] = None,
    ) -> None: ...
    def submit(
        self,
        frame: VideoFrame,
    ...
    def submit_eos(self, eos: EndOfStream, timeout_ms: int = ...) -> bool: ...
    def try_submit_eos(self, eos: EndOfStream) -> bool: ...
