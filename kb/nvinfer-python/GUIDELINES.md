...
- **Buffer ownership** — `NvInfer.submit()` consumes the `gst::Buffer` from the `SharedBuffer` (via `take_inner()`). After calling it, the `SharedBuffer` is consumed (`is_consumed` is true). All outstanding references (SurfaceView, batch objects) must be deleted before consumption, otherwise the engine raises `RuntimeError: SharedBuffer has outstanding references`.
- **PTS correlation** — Pipeline correlation is managed via the `batch_id` parameter on `submit()`. `SavantIdMeta` attached to the buffer provides per-frame identity.
