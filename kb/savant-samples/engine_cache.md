...
```
savant_samples/assets/cache/<model-name>/<platform-tag>/<engine-file>
```
...
- `savant_samples::assets::engine_cache_platform_tag(gpu_id)` — returns
  the platform tag string.
- `savant_samples::assets::engine_cache_dir(model_name, gpu_id)` — creates
  (if missing) and returns the directory above.
- `savant_samples::cars_tracking::model::yolo11n_engine_cache_path(gpu_id)`
...
- **Do not commit cached engines** — they are multi-MB binary blobs
  ...
  `savant_samples/assets/cache/`.
- If reuse mysteriously stops working, remove the whole
  `savant_samples/assets/cache/<model-name>/` tree and let the next run
