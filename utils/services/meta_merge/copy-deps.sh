...
if ! install -m 644 /opt/savant-rs/services/meta_merge/assets/configuration.json "${TARGET_ETC_DIR}/configuration.json"; then
    echo "Error: Failed to copy configuration file" >&2
    exit 1
fi

# Copy Python modules
if ! install -m 644 /opt/savant-rs/services/meta_merge/assets/python/module.py "${TARGET_MODULE_DIR}/"; then
