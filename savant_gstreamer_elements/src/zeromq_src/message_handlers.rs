...
use super::{ZeromqSrc, SAVANT_EOS_EVENT_NAME, SAVANT_EOS_EVENT_SOURCE_ID_PROPERTY};
...
impl ZeromqSrc {
    fn get_frame_key_info_as_string(&self, frame: &VideoFrameProxy) -> String {
    ...
    fn handle_video_frame_impl(
        ...
        gst::debug!(
            ...
            imp = self,
            "Received frame [{}]",
            self.get_frame_key_info_as_string(frame)
        );
    ...
    pub(crate) fn handle_eos(&self, eos: &EndOfStream) -> OptionalGstFlowReturn {
        gst::info!(
            CAT,
            imp = self,
        ...
        self.send_custom_stream_eos_downstream(&eos.source_id)
    }

    fn send_custom_stream_eos_downstream(&self, source_id: &str) -> OptionalGstFlowReturn {
        let savant_eos_event = build_savant_eos_event(source_id);
