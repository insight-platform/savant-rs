...
use rocksdb::{Cache, Options, WriteOptions, DB};
use std::cmp::Ordering;
use utilities::{index_to_key, next_index, u64_from_byte_vec};
...
const ROW_CACHE_SIZE: usize = 512;
...
impl PersistentQueueWithCapacity {
    pub fn new(
        ...
        db_opts.set_row_cache(&Cache::new_lru_cache(ROW_CACHE_SIZE));
        ...
        Ok(Self {
            ...
            high_watermark: (high_watermark as f64 / 100.0 * max_elements as f64).ceil() as usize,
            empty,
        })
    }
...
mod tests {
    ...
    fn test_limit_len_ops() {
        ...
        {
            let mut db =
                PersistentQueueWithCapacity::new(&path, 2, 90, Options::default()).unwrap();
            db.push(&[&[1, 2, 3]]).unwrap();
            assert!(!db.is_high_utilization());
            db.push(&[&[4, 5, 6]]).unwrap();
    ...
    fn test_read_with_close() {
        ...
        {
            let mut db =
                PersistentQueueWithCapacity::new(&path, size, 90, Options::default()).unwrap();
            db.push(&[&[1, 2, 3]]).unwrap();
            db.push(&[&[4, 5, 6]]).unwrap();
            db.push(&[&[7, 8, 9]]).unwrap();
        }
    ...
    fn push_pop_many() {
        ...
        _ = PersistentQueueWithCapacity::remove_db(&path);
        let mut queue = PersistentQueueWithCapacity::new(&path, 3, 90, Options::default()).unwrap();
        queue
