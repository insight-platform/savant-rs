...
//!
//! The stage owns GStreamer muxer lifecycle and maps framework
//! encoded envelopes onto an MP4 file:
//!
//! * Constructs [`Mp4Muxer`] on [`Actor::started`] (so the GStreamer
//!   pipeline initialisation runs on the worker thread).
//! * Maps [`EncodedMsg::Frame`] to
//!   [`savant_gstreamer::mp4_muxer::Mp4Muxer::push`], reading
...
//!   [`savant_gstreamer::mp4_muxer::Mp4Muxer::finish`].
//! * Ignores [`EncodedMsg::StreamInfo`] / [`EncodedMsg::Packet`]
//!   at the `debug` log level — those variants are not part of
//!   the muxer's wire contract but accepted as non-fatal for
//!   forward-compatibility with upstream stages that might emit
//!   them.
//!
//! # Grouped builder API
//!
...
//! — the stage does not add its own cooperative-exit code.
//!
//! # Runtime invariant
//!
...
//! In particular `on_frame` is always installed (with a no-op
//! default if the user does not register one), so the runtime
//! never has to branch on whether the observer is present.
...
/// message kind the stage interprets beyond the load-bearing
...
mod tests {
    ...
    /// invariant that built-in `finish` runs before the user hook.
    #[test]
