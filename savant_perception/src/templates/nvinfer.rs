...
//!   See [`NvInferBuilder::stopping`] for the composition contract
//!   (built-in `graceful_shutdown` + drain-dispatch always run
//!   first; the user hook runs after).
//!
...
///   (template requests a stop via [`HookCtx::request_stop`]).
...
///   outputs.  Default.
/// * [`ErrorAction::Fatal`] — template escalates: requests a
...
///
/// The template emits a single `error!` log line (prefixed by the
...
impl NvInfer {
    ...
    /// template emits a single `error!` line prefixed by the
...
/// * `on_error` — the template emits the `error!` log line, then
...
/// [`NvInferBuilder::common`].  Field semantics match the legacy
...
mod tests {
    ...
    /// downstream (the template emits the log line itself before
    /// invoking the hook).
    #[test]
