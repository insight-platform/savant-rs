...
//!   so the legacy `attach_frame_id_overlay` idiom moves to a
//!   per-frame loop inside the hook body (user-maintained frame
//!   counter — the framework no longer injects one).
...
//! non-`Option` boxed closure — previously-optional
//! `preprocess: Option<FramePreprocessor>` and
//! `src_rect: Option<SrcRectProvider>` are now always installed
//! with the matching `Picasso::default_*` no-op.  The former
//! `on_frame_preprocess` hook was renamed to
//! [`PicassoInboxBuilder::on_delivery`] and promoted to a
//! batch-level delivery hook; the framework-injected
//! `frame_counter` parameter is gone — maintain your own counter
//! in user code (e.g. via an `Arc<AtomicU64>`) if you need a
//! stable per-frame index.
...
/// consuming the batch — the template still owns the underlying
...
/// the shape of
/// [`FunctionInboxBuilder::on_delivery`](super::function::FunctionInboxBuilder::on_delivery)
/// so the cross-template hook vocabulary is uniform.
...
/// template's load-bearing cleanup cannot be skipped — users that
...
/// [`PicassoEngine`](picasso::prelude::PicassoEngine).  Hook slots
...
impl Picasso {
    ...
    /// frame, preserving the legacy "use the whole source surface"
    /// behaviour.  Always installed when the user omits the inbox
    /// setter.
...
impl Picasso {
    fn send_pairs(
        ...
        // shared reference.  Errors are logged but non-fatal to
        // preserve the legacy per-frame preprocessor behaviour.
...
impl PicassoInboxBuilder {
    ...
    /// the loop (historical behaviour inherited from the legacy
    /// per-frame preprocessor — see
    /// [`Picasso::send_pairs`](Picasso)).
...
mod tests {
    ...
    /// `engine.shutdown()` runs before the user hook) is covered
    /// by the `cars-demo` smoke run which exercises a real
    /// `PicassoEngine`.
    #[test]
