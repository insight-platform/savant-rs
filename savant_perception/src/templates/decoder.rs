...
//! # Grouped builder API
//!
//! Hooks are grouped into three bundles following the cross-template
//! pattern:
//!
...
//!   [`ErrorAction`](crate::ErrorAction), matching the
//!   other templates' egress-hook contract.  Because these run off
...
//!   [`PipelineMsg::Delivery`](crate::envelopes::PipelineMsg::Delivery)
//!   via `router.send(..)`.
...
//!   level and returns [`ErrorAction::LogAndContinue`] — preserving
//!   the legacy "one misbehaving stream does not tear the pipeline
//!   down" contract.
...
//! # Runtime invariant
//!
//! Every hook slot on the runtime [`Decoder`] struct and the
//! pool-callback [`VariantHooks`] bundle is a non-`Option` closure
//! ([`StreamInfoObserver`] for the actor-thread observer,
//! `Arc<dyn Fn>` for the pool-callback-thread hooks).  The
//! bundle builders' `Option<...>` fields exist purely as internal
//! "was the setter called?" markers — every bundle's `build()`
//! always substitutes the matching `Decoder::default_on_*` before
//! the actor value is constructed.
...
///   the other templates' egress-error contract.
...
impl Decoder {
    ...
    /// invariant is preserved without any sample-level bookkeeping.
    ...
    /// The error is otherwise swallowed so one misbehaving stream
    /// does not tear the pipeline down; override to return
    /// [`ErrorAction::Fatal`] when the pipeline should stop on any
    /// decoder error.
...
/// shutdown hint from [`EncodedMsg::Shutdown`](crate::envelopes::EncodedMsg::Shutdown)
...
/// contract of the other templates.
fn build_pool(
