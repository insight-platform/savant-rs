...
///   normally.
/// * `Ok(Flow::Stop)` — request cooperative shutdown: the template
...
/// [`HookCtx`].
///
/// The `&Router<EncodedMsg>` parameter was added in the unified
/// error-hook wave; default closures ignore it.  The return
/// [`ErrorAction`] drives the template's exit semantics:
/// `Fatal` latches the error and aborts the default sink
/// (equivalent to today's behaviour), `LogAndContinue` records the
...
/// observe the demuxer's post-drain state.  The template's own
